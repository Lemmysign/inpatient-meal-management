...
import '../../styles/kitchenstaff/kitchendinner.css';
...
const KitchenDinner: React.FC = () => {
  ...
  const fetchDinnerQueue = useCallback(async () => {
    try {
      setIsLoading(true);
      ...
      const response = await fetch(`${config.kitchenDinnerQueueUrl}?${params.toString()}`, {
        headers: {
      ...
      if (result.success) {
        ...
        setError(result.message || 'Failed to load dinner queue');
      }
    } catch {
      setError('Failed to load dinner queue. Please try again.');
    } finally {
      setIsLoading(false);
    }
  }, [currentPage, searchQuery, statusFilter]);

  useEffect(() => { fetchDinnerQueue(); }, [fetchDinnerQueue]);

  // Queue auto-refresh every 2 minutes
  useEffect(() => {
    const id = setInterval(fetchDinnerQueue, 120000);
        ...
        <div className="ks-content-wrapper">
          ...
          <div className="ks-welcome">
            <div className="ks-page-header">
              <div>
                <h1 className="ks-welcome-title">Dinner Queue</h1>
                <p className="ks-welcome-subtitle">
                  <span className="ks-role-badge">DINNER</span>
