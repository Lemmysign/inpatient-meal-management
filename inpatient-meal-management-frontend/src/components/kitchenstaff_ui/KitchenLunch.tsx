...
import '../../styles/kitchenstaff/kitchenlunch.css';
...
const KitchenLunch: React.FC = () => {
  ...
  const fetchLunchQueue = useCallback(async () => {
    try {
      setIsLoading(true);
      ...
      const response = await fetch(`${config.kitchenLunchQueueUrl}?${params.toString()}`, {
        headers: {
      ...
      if (result.success) {
        ...
        setError(result.message || 'Failed to load lunch queue');
      }
    } catch {
      setError('Failed to load lunch queue. Please try again.');
    } finally {
      setIsLoading(false);
    }
  }, [currentPage, searchQuery, statusFilter]);

  useEffect(() => { fetchLunchQueue(); }, [fetchLunchQueue]);

  // Queue auto-refresh every 2 minutes
  useEffect(() => {
    const id = setInterval(fetchLunchQueue, 120000);
        ...
        <div className="ks-content-wrapper">
          ...
          <div className="ks-welcome">
            <div className="ks-page-header">
              <div>
                <h1 className="ks-welcome-title">Lunch Queue</h1>
                <p className="ks-welcome-subtitle">
                  <span className="ks-role-badge">LUNCH</span>
