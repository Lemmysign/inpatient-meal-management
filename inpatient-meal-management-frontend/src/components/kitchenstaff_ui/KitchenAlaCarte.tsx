...
import '../../styles/kitchenstaff/kitchenlacarte.css';
...
const KitchenAlaCarte: React.FC = () => {
  ...
  const fetchAlaCarteQueue = useCallback(async () => {
    try {
      setIsLoading(true);
      ...
      const response = await fetch(`${config.kitchenAlaCarteQueueUrl}?${params.toString()}`, {
        headers: {
      ...
      if (result.success) {
        ...
        setError(result.message || 'Failed to load à la carte queue');
      }
    } catch {
      setError('Failed to load à la carte queue. Please try again.');
    } finally {
      setIsLoading(false);
    }
  }, [currentPage, searchQuery, statusFilter]);

  useEffect(() => { fetchAlaCarteQueue(); }, [fetchAlaCarteQueue]);

  // Queue auto-refresh every 2 minutes
  useEffect(() => {
    const id = setInterval(fetchAlaCarteQueue, 120000);
        ...
        <div className="ks-content-wrapper">
          ...
          <div className="ks-welcome">
            <div className="ks-page-header">
              <div>
                <h1 className="ks-welcome-title">À la carte Queue</h1>
                <p className="ks-welcome-subtitle">
                  <span className="ks-role-badge">EXTRA</span>
          ...
          <div className="ks-card">
            {isLoading ? (
              ...
              <div className="ks-empty-state">
                <ShoppingBag size={48} />
                <p>No à la carte orders found</p>
              </div>
            ) : (
              <>
                {/* Desktop Table */}
...
export default KitchenAlaCarte;